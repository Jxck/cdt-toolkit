...
fn dictionary_matches_ruby_oracle() {
  ...
  assert_eq!(candidate.len(), expected.len(), "dictionary byte length differs from Ruby oracle");
  assert_eq!(candidate, expected, "dictionary body differs from Ruby oracle");
}
