...
pub fn run(mut args: CompressArgs) -> Result<()> {
  ...
  for input in inputs {
    ...
    if args.raw_brotli {
      let output = brotli::compress_with_dictionary(&data, &dict, BROTLI_QUALITY, BROTLI_WINDOW)?;
    ...
    if args.dcb {
      let raw = brotli::compress_with_dictionary(&data, &dict, BROTLI_QUALITY, BROTLI_WINDOW)?;
